...
test("pauses and unpauses the dashboard queue", async ({ page }) => {
  await installDashboardSettingsFixtures(page);

  await page.goto("/dashboard/home.html#/queue");

  await expect(page.locator("#dashboard-page-title")).toHaveText("Dashboard Queue");
  await expect(page.locator("#dashboard-queue-summary-body")).toBeHidden();

  await page.locator("#dashboard-queue-summary-collapse-toggle").click();
  await expect(page.locator("#dashboard-queue-summary-body")).toBeVisible();
