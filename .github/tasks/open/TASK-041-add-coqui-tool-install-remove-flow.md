...
Verified 2026-04-21: `npm run test:e2e -- e2e/tools-coqui-tts.spec.ts` passes in chromium/firefox/webkit against the served app. The browser gate now uses a local managed `tts` command fixture through the real backend install/remove endpoints so coverage no longer depends on public `TTS` package availability.
...
Verified 2026-04-22: `bash app/scripts/test-real-failfast.sh` exits 0 once localhost PostgreSQL access is allowed for the sandboxed command. `app/tests/test-artifacts/system-result.json` now reports `{ "step": "all", "exit_code": 0, "command": "test-system.sh" }`.
