...
1. **(R-TEST-009)** Run `app/scripts/test-real-failfast.sh` before any broader gate. It delegates to `test-system.sh`, which runs environment bootstrap, DB reset, `test_startup_lifecycle.py`, the full non-smoke pytest suite, and the minimal critical browser subset; it writes a machine-readable artifact to `app/tests/test-artifacts/system-result.json` on failure, including bootstrap-time prerequisite failures.
2. Stubbed Playwright coverage for first-party backend workflows is prohibited. Playwright browser tests must run against the real FastAPI app and real PostgreSQL test database, and isolated frontend-only state/rendering checks belong in Vitest instead of Playwright.
...
| R-TEST-009 | AI agents must use `app/scripts/test-real-failfast.sh` as the first-pass command for minimal-context real-test verification before running broader gates; Playwright specs for first-party backend workflows must run against the real FastAPI app + real PostgreSQL test DB (no `installDashboardSettingsFixtures` or first-party `page.route()` interception), and frontend-only state/rendering checks belong in Vitest | AI agent test workflow and Playwright authoring |
