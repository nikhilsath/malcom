...
2. Stubbed Playwright coverage (specs using `installDashboardSettingsFixtures` or `page.route()` intercepts for first-party backend routes) is **prohibited**. Browser tests must run against the real FastAPI app and real PostgreSQL test database. Isolated frontend-only state tests that do not need browser/system proof belong in Vitest rather than Playwright. (→ R-TEST-010)
...
| R-TEST-009 | AI agents must use `app/scripts/test-real-failfast.sh` as the first-pass command for minimal-context real-test verification before running broader gates | AI agent test workflow and Playwright authoring |
| R-TEST-010 | Playwright specs must not use `installDashboardSettingsFixtures` or `page.route()` to intercept first-party backend routes; browser tests must run against the real FastAPI app and real PostgreSQL test database; isolated frontend-only state tests belong in Vitest | Playwright authoring and browser test architecture |
