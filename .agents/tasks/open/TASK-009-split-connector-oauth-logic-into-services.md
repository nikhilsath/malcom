...
Action: Update connector API tests to patch/assert against the new service/client boundaries where needed (instead of route-local Google token helpers) while preserving existing API contract assertions for OAuth start/callback/refresh behavior.
