...
Result: README.md updated to explicitly state the Settings -> Connectors UI must fetch live connector availability from `GET /api/v1/connectors` and must not rely on cached `settings` payloads as authoritative.
