...
> **(R-DB-001)** Schema source of truth is `app/backend/database.py`. **(R-DB-002)** Documentation in `AGENTS.md` and `README.md` must stay aligned with `app/backend/database.py` when tables or table groups change.

### Source Of Truth

The repo-facing schema structure source of truth is `app/backend/database.py`.
Alembic migrations under `app/backend/migrations/` are the required structural change log and execution path, and they must stay aligned with `app/backend/database.py`.
...
- connection helper: `app/backend/database.py:connect`
- initialization entrypoint: `app/backend/database.py:initialize`
...
1. update `app/backend/database.py`
2. add a new migration under `app/backend/migrations/versions/` for table/column changes
3. keep `app/backend/database.py:run_migrations()` and initialization flow aligned with migration ownership
...
1. Keep provider-specific, connector-specific, or integration-specific behavior in scoped service modules (for example `app/backend/services/connector_<provider>*.py`) instead of growing generic route or service files with more branches. (→ R-CODE-001)
...
- routes: `app/backend/routes/<feature>.py`
- service/business logic: `app/backend/services/<feature>.py`
- schema/model changes: `app/backend/schemas.py`
- DB helpers and schema: `app/backend/database.py`
- app wiring and mounts: `app/backend/main.py`
- UI route wiring and redirects: `app/backend/routes/ui.py`
