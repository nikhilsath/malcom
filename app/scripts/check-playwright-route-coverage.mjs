...
const registryPath = path.join(workspaceRoot, "app", "ui", "page-registry.json");
const coveragePath = path.join(workspaceRoot, "app", "ui", "e2e", "coverage-route-map.json");
...
const validateOwners = (owners, routePath, bucketLabel, invalidEntries) => {
  ...
  owners.forEach((owner, ownerIndex) => {
    ...
    const specPath = path.join(workspaceRoot, "app", "ui", "e2e", spec);
